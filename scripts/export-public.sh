...
sed -i "s/const KNOWN_THEMES = \[.*\];/const KNOWN_THEMES = ['default', 'example', 'example-cliff'];/" "$PUBLIC_DIR/admin.html"
...
# Always copy default theme + the generic cliff example (no real operator data)
cp "$PRIVATE_DIR/themes/default.json"      "$PUBLIC_DIR/themes/default.json"
cp "$PRIVATE_DIR/themes/example-cliff.json" "$PUBLIC_DIR/themes/example-cliff.json"
...
echo ""
echo "  Themes included : default, example, example-cliff"
